...
* **Il Laghetto delle Carpe Koi:** Un piccolo specchio d'acqua. Se immergiamo un dito, l'acqua crea increspature circolari (facile da fare con uno shader in Three.js). Le carpe Koi nuotano usando un algoritmo *Boids* (flocking) e, se tieniamo la mano a pelo d'acqua, si avvicineranno curiose.
...
* **Lighting Estimation API:** Utilizzare le API per la stima dell'illuminazione reale. Se l'utente gioca in una stanza buia, il giardino rifletterà quell'illuminazione; se è in una stanza luminosa, i modelli 3D riceveranno una luce coerente con l'ambiente reale, fondendosi perfettamente.

---

## 6. Generazione Procedurale degli Asset in Three.js (Ottimizzazione)

Poiché il visore deve elaborare tutto con un chip mobile Snapdragon, ecco come mantenere l'esperienza fluida in WebGL/Three.js. Per mantenere l'applicazione ultraleggera (zero tempi di caricamento) e aderente allo stile visivo "Low-Poly", **nessun asset esterno (.gltf, .obj) dovrà essere importato**. Tutti i modelli 3D dovranno essere costruiti programmaticamente tramite le primitive di Three.js.
